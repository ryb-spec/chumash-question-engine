...
def test_all_gates_false_and_internal_decisions_exact():
    p=json.loads(validator.PACKET_JSON.read_text(encoding="utf-8")); assert p["fake_review_decisions_created"] is False; assert p["fake_observations_created"] is False; assert p["fake_student_data_created"] is False; assert p["source_truth_changed"] is False; assert p["question_selection_changed"] is False; assert p["scoring_mastery_changed"] is False
    for i in p["items"]:
        assert i["internal_review_decision"]==validator.EXPECTED_DECISIONS[i["source_candidate_id"]]
        assert i["observation_result"] is None
