...
BATCH_004_PREVIEW_PATH = (
    ROOT
    / "data"
    / "curriculum_extraction"
    / "generated_questions_preview"
    / "batch_004_preview.jsonl"
)
BATCH_004_PREVIEW_RELATIVE = "data/curriculum_extraction/generated_questions_preview/batch_004_preview.jsonl"
BATCH_005_PREVIEW_PATH = (
    ROOT
    / "data"
    / "curriculum_extraction"
    / "generated_questions_preview"
    / "batch_005_preview.jsonl"
)
BATCH_005_PREVIEW_RELATIVE = "data/curriculum_extraction/generated_questions_preview/batch_005_preview.jsonl"
...
BATCH_004_EXPECTED_COUNTS = {
    "phrase_translation": 50,
    "hebrew_to_english_match": 25,
    "english_to_hebrew_match": 25,
}
BATCH_005_EXPECTED_COUNTS = {
    "phrase_translation": 50,
    "hebrew_to_english_match": 25,
    "english_to_hebrew_match": 25,
}
...
def test_preview_v1_is_preserved_and_v2_exists_with_minimum_count() -> None:
    ...
    assert BATCH_004_PREVIEW_PATH.exists()
    assert BATCH_005_PREVIEW_PATH.exists()
...
def test_batch_004_preview_exists_with_minimum_count() -> None:
    records = load_jsonl(BATCH_004_PREVIEW_PATH)
    assert len(records) >= 100


def test_batch_005_preview_exists_with_minimum_count() -> None:
    records = load_jsonl(BATCH_005_PREVIEW_PATH)
    assert len(records) >= 100
...
def test_batch_004_preview_records_are_valid_jsonl_with_required_fields() -> None:
    records = load_jsonl(BATCH_004_PREVIEW_PATH)
    record_ids = source_record_ids()
    prompts: set[str] = set()
    counts = Counter(record["question_type"] for record in records)
    ...
    assert dict(counts) == BATCH_004_EXPECTED_COUNTS


def test_batch_005_preview_records_are_valid_jsonl_with_required_fields() -> None:
    records = load_jsonl(BATCH_005_PREVIEW_PATH)
    record_ids = source_record_ids()
    prompts: set[str] = set()
    counts = Counter(record["question_type"] for record in records)
    ...
    assert dict(counts) == BATCH_005_EXPECTED_COUNTS
...
def test_preview_validator_accepts_generated_preview() -> None:
    ...
    assert summary["preview_file_count"] == 6
    assert summary["preview_record_count"] == 710
    ...
    assert summary["preview_file_question_type_counts"][BATCH_004_PREVIEW_RELATIVE] == BATCH_004_EXPECTED_COUNTS
    assert summary["preview_file_question_type_counts"][BATCH_005_PREVIEW_RELATIVE] == BATCH_005_EXPECTED_COUNTS
